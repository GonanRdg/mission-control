...
/**
 * The painted frame art comes in two cuts: the near-black originals, and the
 * `-light` variants generated by scripts/generate-light-borders.mjs. Which cut
 * is in play depends on the appearance, NOT the accent — but these have to be
 * written as inline styles (they are per-accent URLs, which CSS cannot build),
 * so re-applying them is part of both an accent change and a theme change.
 *
 * button_filled_* has no light cut: a solid accent CTA already reads correctly
 * on paper, so it always points at the original.
 */
export function frameArtVars(
  id: string | null | undefined,
  theme: "dark" | "light",
): Record<string, string> {
  const accent = getAccentColor(id).id;
  const cut = theme === "light" ? "-light" : "";
  return {
    "--mc-btn-filled-image": `url("/borders/button_filled_${accent}.png")`,
    "--mc-panel-focused-image": `url("/borders/panel_focused_${accent}${cut}.png")`,
    "--mc-panel-image": `url("/borders/square_${accent}${cut}.png")`,
    "--mc-shell-image": `url("/borders/shell_${accent}${cut}.png")`,
  };
}

/** Read the accent + appearance off the DOM and rebind the frame art. Called
 *  on an accent change and on a dark/light flip. */
export function applyFrameArt(id?: string | null): void {
  ...
  if (accent === undefined) {
    try {
      accent = window.localStorage.getItem(ACCENT_CACHE_KEY);
    } catch {
      accent = null;
    }
  }
  for (const [key, value] of Object.entries(frameArtVars(accent, theme))) {
