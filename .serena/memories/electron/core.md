...
- **Updater**: `update-manager.ts` — **disabled in this fork** (`updatesDisabled`, no `publish` feed, update UI and release poll removed). Short-circuits on a flag rather than being deleted so upstream stays mergeable; see `mem:release`.
